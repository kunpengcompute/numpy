...
- `full_ut.sh` -> 新增 `full_ut`，只跑全量单元测试，不采集覆盖率
...
- `full_ut.sh`
  - `FULL_UT_ARGS`: 仅传给 `pytest` 的参数；不要包含 `--cov`、`--cov-report`、`--gcov` 或 `--gcov-format`
  - `FULL_UT_BUILD_ARGS`: 传给 `spin build --clean --` 的 Meson 参数
...
- `full_ut.sh` 复用 `full.sh` 的 full suite 范围、`-k` 过滤和 `--deselect` 列表，但不传 `--gcov`、`--cov` 或 `--cov-report`，也不会生成或检查覆盖率。性能优化 PR 如果只要求“全量 UT、不跑覆盖率”，应使用该脚本作为门禁证据。
